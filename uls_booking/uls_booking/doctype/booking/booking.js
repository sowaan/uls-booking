...
frappe.ui.form.on("Booking", {
    ...
    onload : async function(frm)
    {
        ...
        frm.set_df_property("duty_tax_forwarding", "read_only", 1);
        frm.set_df_property("residential_surcharge", "read_only", 1);
    ...
    customer: async function(frm)
    {
        if (frm.doc.customer)
        {
            ...
            await frappe.call({
                method: "uls_booking.uls_booking.api.api.get_icris_accounts",
                args: {
                    customer: frm.doc.customer,
                },
                async: false,
        ...
        if (frm.doc.customer)
            {
                    ...
                    if ( c.custom_import_control == 1 )
                    {
                        frm.set_df_property("ic_label", "read_only", 0);
                    }
                    if ( c.custom_return_electronic_label == 1 )
                    {
                        frm.set_df_property("return_electronic_label", "read_only", 0);
                    }     
                    if ( c.custom_allow_saturday_delivery == 1 )
                    {
                        frm.set_df_property("saturday_delivery", "read_only", 0);
                    }
                    if ( c.custom_shipping_bill_charges_applicable == 1 )
                    {
                        frm.set_df_property("shipping_bill_charges", "read_only", 0);
                    }
                    if ( c.custom_allow_direct_delivery_only == 1 )
                    {
                        frm.set_df_property("direct_delivery", "read_only", 0);
                    }    
                    if ( c.custom_dutytax_forwarding_surcharge == 1 )
                    {
                        frm.set_df_property("duty_tax_forwarding", "read_only", 0);
                    }
                    if ( c.custom_allow_delivery_confirmation_signature == 1 )
                    {
                        frm.set_df_property("signature_options", "read_only", 0);
                    }
                    if ( c.custom_residential_surcharge == 1 )
                    {
                        frm.set_df_property("residential_surcharge", "read_only", 0);
                    }
