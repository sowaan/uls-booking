...
frappe.ready(async function() {
	...
	frappe.web_form.on('customer', async (field, value) => {
		...
		if (customer) {
			...
			{
				await frappe.call({
					method: "uls_booking.uls_booking.api.api.get_service_types",
					args: {
						customer : customer,
			...
			{
					await frappe.call({
						method: "uls_booking.uls_booking.api.api.get_credit_balance",
						args: {
							customer : customer,
							company : company,
						},
	...
	//         await frappe.call({
	//             method: "uls_booking.uls_booking.api.api.get_cities",
	//             args: {
	//                 country : country,
	//             },
	//             callback: function (r) {
	...
	frappe.web_form.on('consignee_country', async (field, value) => {
		...
		if (country)
		{
			...
			await frappe.call({
				method: "uls_booking.uls_booking.api.api.get_postal_codes",
				args: {
					// city : city,
					country : country, 
				},
